...
import { compareOperatorOptions, dataTypeOptions } from "../../types/pages/layout-management.types";
...
interface LayoutModalProps {
  ...
  onMappingFieldChange: (rowId: string, event: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => void;
  onAddMapping: () => void;
          ...
          <form id="layout-form" onSubmit={onSubmit} className="space-y-8">
            ...
            <div>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <InputField label="Nombre" name="name" value={layoutForm.name} onChange={onFieldChange} required />
                <InputField label="Threshold auto-match" name="autoMatchThreshold" value={layoutForm.autoMatchThreshold} onChange={onFieldChange} required />
                <InputField label="Etiqueta sistema" name="systemLabel" value={layoutForm.systemLabel} onChange={onFieldChange} required />
                <InputField label="Etiqueta banco" name="bankLabel" value={layoutForm.bankLabel} onChange={onFieldChange} required />
              </div>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <label className="space-y-1.5">
                  <span className="text-sm font-semibold text-slate-700">Descripcion</span>
                  <input name="description" value={layoutForm.description} onChange={onFieldChange} className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none" />
                </label>
                <label className="flex items-center gap-2 self-end rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-700">
                  <input type="checkbox" name="active" checked={layoutForm.active} onChange={onFieldChange} />
            ...
            <div className="rounded-2xl border border-slate-200">
              ...
              <div className="space-y-5 p-5 lg:p-6">
                {layoutForm.mappings.map((mapping, index) => (
                  <div key={mapping.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-5 lg:p-6">
