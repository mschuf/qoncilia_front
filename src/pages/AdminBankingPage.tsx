              ...
              {selectedBank ? (
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  <BankContextTile
                    ...
                    helper={selectedBank.alias ? `Alias ${selectedBank.alias}` : "Sin alias"}
                  />
                  <BankContextTile
                    icon={FiUsers}
                    label="Responsable"
                    value={selectedBank.userLogin}
                    helper="Usuario asignado al banco"
                  />
                  <BankContextTile
                    icon={FiMapPin}
                    label="Sucursal"
                    value={selectedBank.branch ?? "Sin sucursal"}
                    helper="Dato definido en la ficha del banco"
                  />
                  <BankContextTile
                    icon={FiCreditCard}
                    label="Cuentas"
                    value={String(visibleAccounts.length)}
                    helper={selectedBank.active ? "Banco activo" : "Banco inactivo"}
                    accent={selectedBank.active ? "emerald" : "amber"}
                  />
                </div>
              ) : (
                <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="mt-2 flex flex-wrap gap-2 text-xs">
                            ...
                            <span className={`rounded-full px-2.5 py-1 font-semibold ${
                              account.active ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                            }`}>
